...
# --- 🧠 INICIALIZACIÓN DE SESIÓN (BASE DE DATOS ANTI-LAG) ---
if 'c_sup' not in st.session_state: st.session_state.c_sup = 1
if 'c_caj' not in st.session_state: st.session_state.c_caj = 2
if 'c_coc' not in st.session_state: st.session_state.c_coc = 3
if 'c_sal' not in st.session_state: st.session_state.c_sal = 4
if 'c_bar' not in st.session_state: st.session_state.c_bar = 2
if 'c_emp' not in st.session_state: st.session_state.c_emp = 2
if 'c_aux' not in st.session_state: st.session_state.c_aux = 2
if 'c_hos' not in st.session_state: st.session_state.c_hos = 2
...
if 'df_ventas' not in st.session_state:
    st.session_state.df_ventas = pd.DataFrame({"Día": dias_semana, "Venta Proyectada ($)": [20000.0] * 7})

if 'df_fijos_dict' not in st.session_state:
    st.session_state.df_fijos_dict = {}
    for p in puestos_fijos:
        df_temp = pd.DataFrame([{"Día": d, "Matutino": False, "Intermedio": True, "Vespertino": False} for d in dias_semana])
...
if 'df_demanda' not in st.session_state:
    ...
    for d in dias_semana:
        for b in bloques:
            filas_dem.append({"Día": d, "Bloque": b, "🍳 Cmds Cocina": 15.0, "🍳 Extra Cocina": 0.0, "🍔 Cmds Salón": 20.0, "🍔 Extra Salón": 0.0, "🍺 Cmds Barra": 5.0, "🍺 Extra Barra": 0.0})
    st.session_state.df_demanda = pd.DataFrame(filas_dem)

if 'db' not in st.session_state:
    st.session_state.db = {'ventas': {}, 'fijos': {}, 'demanda': {}}
    for d in dias_semana:
        st.session_state.db['ventas'][d] = 20000.0
        st.session_state.db['fijos'][d] = {'sm': False, 'si': True, 'sv': False, 'cm': True, 'ci': False, 'cv': True, 'hm': False, 'hi': True, 'hv': True, 'em': False, 'ei': True, 'ev': True, 'am': False, 'ai': True, 'av': True}
        st.session_state.db['demanda'][d] = {'cc': [15.0]*5, 'ec': [0.0]*5, 'cs': [20.0]*5, 'es': [0.0]*5, 'cb': [5.0]*5, 'eb': [0.0]*5}
...
def update_all_fijos(puesto, turno, valor):
    ...
    if f"editor_fijos_{puesto}" in st.session_state: 
        st.session_state[f"editor_fijos_{puesto}"] = {"edited_rows": {}, "added_rows": [], "deleted_rows": []}
...
# LÓGICA DE INYECCIÓN ESTRICTA (PRIMER Y ÚLTIMO BLOQUE ÚNICAMENTE)
def inyectar_horas_extra(dia_in, turno_in, area_in, hrs_in):
    ...
    # Asignación estricta al primer bloque o al último bloque
...
with st.sidebar:
    ...
    if not st.session_state['config_unlocked']:
        ...
        with st.expander("💵 Salarios y Capacidad"):
            for k in ['s_coc','s_ven','s_bar','s_sup','s_caj','s_hos']: config_data[k] = st.number_input(f"{k}", value=config_data[k])
        if st.button("🔒 Guardar y Bloquear", type="primary"):
            save_config(config_data); st.session_state['config_unlocked'] = False; st.rerun()
    st.markdown("---")
...
with tab_carga:
    ...
    with t_equ:
        ...
        with c_rh1:
            st.session_state['c_sup'] = st.number_input("⭐️ Supervisor", 0, value=st.session_state['c_sup'])
            st.session_state['c_caj'] = st.number_input("🖥️ Caja", 0, value=st.session_state['c_caj'])
        with c_rh2:
            st.session_state['c_coc'] = st.number_input("🍳 Cocinero", 0, value=st.session_state['c_coc'])
            st.session_state['c_sal'] = st.number_input("🍔 Vendedor", 0, value=st.session_state['c_sal'])
        with c_rh3:
            st.session_state['c_bar'] = st.number_input("🍺 Barra", 0, value=st.session_state['c_bar'])
            st.session_state['c_emp'] = st.number_input("📦 Empacador", 0, value=st.session_state['c_emp'])
        with c_rh4:
            st.session_state['c_aux'] = st.number_input("🧹 Auxiliar", 0, value=st.session_state['c_aux'])
            st.session_state['c_hos'] = st.number_input("🛎️ Hostes", 0, value=st.session_state['c_hos'])
    ...
    with t_dem:
        ...
        # SÓLO 2 OPCIONES: MATUTINO O VESPERTINO
    ...
    if st.button("🚀 CALCULAR PLANTILLA IDEAL (CLICK AQUÍ)", type="primary", use_container_width=True):
        ...
        # --- BLINDAJE ABSOLUTO DE BASE DE DATOS ---
        st.session_state.db = {'ventas': {}, 'fijos': {}, 'demanda': {}}
        for d in dias_semana:
...
with tab_semanal:
    if st.session_state['resultados_diarios'] is not None:
        ...
        for d in dias_semana:
            ...
            n_dia = d + (" ⭐" if res['Es_Especial'] else "") + (" 🏖️" if d == st.session_state.get('descanso_sup', '') else "")
        ...
        def color_filas(row):
            if "📌 TOTAL" in str(row['Día']): return ['background-color: #FF9800; color: white; font-weight: bold;'] * len(row) 
            elif "⭐" in str(row['Día']): return ['background-color: rgba(255, 215, 0, 0.25)'] * len(row) 
            elif any(d in str(row['Día']) for d in dias_alternos): return ['background-color: rgba(130, 130, 130, 0.20)'] * len(row) 
            else: return [''] * len(row) 
        
        st.dataframe(df_maestra.style.set_properties(**{'text-align': 'center'}).apply(color_filas, axis=1), height=830, use_container_width=False, hide_index=True, column_config={"Día": st.column_config.TextColumn("Día", width=200)})
