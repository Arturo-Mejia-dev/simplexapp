...
# --- 🎨 INYECCIÓN DE CSS (ANIMACIÓN, CARGA AZUL, TARJETAS Y PDF) ---
st.markdown("""
<style>
...
[data-testid="stSkeleton"], .stSkeleton {
    ...
    border-radius: 15px !important; opacity: 0.9 !important; border: 2px solid rgba(255, 215, 0, 0.3); box-shadow: 0 0 15px rgba(255, 0, 0, 0.2); 
}
@keyframes mega_shimmer { 0% { background-position: 300% 0; } 100% { background-position: -300% 0; } }

/* 3. MEGA-SPINNER AMARILLO CENTRADO */
div.stSpinner > div > div { border-color: #FFD700 transparent transparent transparent !important; width: 80px !important; height: 80px !important; border-width: 8px !important; }
div.stSpinner { text-align: center; margin-top: 50px; font-size: 20px; font-weight: bold; color: #FFD700; }

/* 4. ESTILO ESPECIAL PARA EL BOTÓN CUANDO ESTÁ 'RUNNING' */
.stButton > button:disabled { border: 5px solid #FF0000 !important; animation: pulse_red_border 1s infinite !important; background-color: #330000 !important; color: white !important; font-weight: bold !important; }
@keyframes pulse_red_border { 0% { box-shadow: 0 0 0 0 rgba(255, 0, 0, 0.7); } 70% { box-shadow: 0 0 0 20px rgba(255, 0, 0, 0); } 100% { box-shadow: 0 0 0 0 rgba(255, 0, 0, 0); } }

/* 5. FONDO AZUL PARA EL CARGADOR DE EXCEL */
[data-testid="stFileUploader"] { background-color: #EBF5FB !important; border: 2px dashed #2E86C1 !important; border-radius: 10px !important; padding: 15px !important; }
[data-testid="stFileUploader"] label { color: #154360 !important; font-weight: bold !important; }
...
salarios_map = {
    'Supervisor': s_sup, 'Caja': s_caj, 'Cocinero': s_coc, 'Vendedor': s_ven, 
    'Barra': s_bar, 'Empacador': s_emp, 'Auxiliar': s_aux, 'Hostes': s_hos
}
...
def render_tab_diario():
    # --- RESUMEN EJECUTIVO (TEMA AZUL) AL INICIO ---
    st.markdown("""
    ...
    # --- GRÁFICOS ---
    st.markdown(f"### 💎 Radiografía del Restaurante")
    ...
    st.caption("💡 **Guía Rápida:** Este gráfico mide el volumen de trabajo en el día. Entre más alta sea la montaña, más clientes hay y más presión recae sobre esa área específica.")
    
    st.markdown(f"### ⚖️ Cobertura de Personal")
    area_grafico = st.radio("Elige el Área Operativa:", ["Cocina", "Salon", "Barra"], horizontal=True, key="sel_area_print")
    
    # --- TOTALES EN GRÁFICA CON CONVERSIÓN A PERSONAS ---
    sum_necesaria = sum(res_dia['req'][area_grafico])
    sum_programada = sum(res_dia['prov'][area_grafico])
    
    # Como cada turno matutino, intermedio y vespertino suma 8 horas de trabajo:
    personas_necesarias = sum_necesaria / 8.0
    personas_programadas = int(sum_programada / 8.0)

    st.markdown(f"""
    <div style="display: flex; gap: 20px; margin-bottom: 10px;">
        <div style="background-color: #fdf2f2; padding: 15px; border-radius: 8px; border: 1px solid #f8d7da; flex: 1;">
            <div style="color: #d62728; font-weight: bold; font-size: 16px;">🔴 Total Horas Necesarias en el Día: {sum_necesaria:.1f} hrs</div>
            <div style="color: #d62728; font-size: 14px; margin-top: 5px;">👥 <i>Equivale a <b>{personas_necesarias:.1f}</b> personas teóricas (turnos de 8 hrs)</i></div>
        </div>
        <div style="background-color: #f2fdf2; padding: 15px; border-radius: 8px; border: 1px solid #d1e7dd; flex: 1;">
            <div style="color: #2ca02c; font-weight: bold; font-size: 16px;">🟢 Total Horas Programadas (Personal): {sum_programada:.1f} hrs</div>
            <div style="color: #2ca02c; font-size: 14px; margin-top: 5px;">👥 <i>Equivale a <b>{personas_programadas}</b> personas reales asignadas hoy</i></div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.caption("💡 **Guía Rápida:** La barra **Roja** es el trabajo que generan los clientes. La barra **Verde** es el personal que el algoritmo asignó. El objetivo perfecto es que la Verde alcance a cubrir o rebase ligeramente a la Roja en cada bloque horario para asegurar que el servicio sea excelente.")

def render_tab_semanal():
    # --- ANÁLISIS EJECUTIVO SEMANAL AL INICIO ---
    st.markdown("""
    <div style="background-color: #FFF3E0; padding: 15px; border-left: 5px solid #FF9800; border-radius: 5px; margin-bottom: 20px;">
        ...
        <p style="font-size: 14px; color: #333;">
            ...
            Al evaluar los 7 días en conjunto, el sistema asegura que la rentabilidad global se mantenga intacta, incluso si tienes "Días Especiales" (donde se infló el presupuesto temporalmente). <br><br>
            Para la <b>Plantilla Maestra Semanal (Tabla Inferior)</b>, el algoritmo consolida todos los turnos diarios. Esta tabla es tu hoja de ruta final: te dice exactamente a cuántos empleados necesitas convocar cada día y en qué turno, garantizando que el gasto final sea el mostrado en los indicadores de arriba.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.caption("💡 **Guía Rápida:** Compara visualmente tus ventas contra tu nómina. Permite detectar de un vistazo los días más rentables (barras verdes altas) contra los días más caros de operar.")
    st.markdown("---")
    ...
    def color_filas(row):
        dia_str = str(row['Día'])
        if "📌 TOTAL" in dia_str: return ['background-color: #FF9800; color: white; font-weight: bold;'] * len(row) # TEMA NARANJA PARA TOTAL
...
def render_tab_ideal():
    # --- ANÁLISIS EJECUTIVO RH (TEMA MORADO) AL INICIO ---
    st.markdown("""
    ...
    st.markdown("---")
    st.markdown("### 📊 Comparativo Gráfico (Ideal vs Real)")
    ...
    st.caption("💡 **Guía Rápida:** Permite comparar visualmente el tamaño actual de tu equipo (Naranja) contra la estructura que las matemáticas exigen para operar perfectamente (Azul).")

# ==========================================
# 🚀 EJECUCIÓN DEL ALGORITMO Y PESTAÑAS PRINCIPALES
# ==========================================
...
if modo_impresion:
    ...
    # --- MODO INTERACTIVO NORMAL CON PESTAÑAS ---
